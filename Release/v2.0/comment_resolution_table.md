...
| 12 | cPP p.22 | Highlighted `user identity` in FAU_SEL.1.1. | Treated as context for duplicate/removal refinement. | Fixed | `Documents/cPP/DBMS_cPP-working_copy.adoc:628` |
| 13 | cPP p.22 | "Refinement: Crossed out, bold text" on FAU_SEL.1.1. | Removed duplicate `user identity` from the selection list because it is already the mandatory first bullet. | Fixed | `Documents/cPP/DBMS_cPP-working_copy.adoc:628` |
...
| 17 | cPP p.25 | "Refinement: Crossed out, bold text" on FMT_MSA.3.2. | Removed `the` before `[no user]`. | Fixed | `Documents/cPP/DBMS_cPP-working_copy.adoc:764` |
