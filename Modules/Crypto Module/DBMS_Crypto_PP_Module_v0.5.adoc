...
|0.4
|2026-07-07
|Package Conformance now requires the TLS FP v2.1 claim (and X.509 FP v1.0 where certificate authentication applies) rather than disclaiming functional package claims; Exact Conformance permits extended components defined in Functional Packages claimed per Package Conformance; Conformance Claim Rationale adds identification of claimed Functional Packages. SFR List application note on FDP_DIT_EXT.1 identifies both the Crypto + Cloud and Crypto + DBaaS review families.
...
|0.4
|2026-07-20
|Refined the ML-KEM model to separate key-pair generation, encapsulation, and decapsulation according to the TOE role. Added selection-based FCS_CKM.1/KEM for TOE-generated ML-KEM key pairs; made FCS_RBG.1 conditional on KeyGen or encapsulation; added the "Established through Key Encapsulation" Master Key origin; and corrected FDP_ITC_EXT.1.4 so ML-KEM establishes a KEK that KeyWrap or AEAD uses to protect transferred keying material rather than treating a KEM as direct key wrapping. Protocol ML-KEM remains governed by the applicable Functional Package. Dependency, rationale, SFR inventory, and ECD text aligned.

|0.5
|2026-07-20
|Closed the remaining priority findings. FCS_CKM_EXT.1.2 now requires an explicit, boundary-aware Master Key protection model and separates TOE protection from supporting operational-environment storage protection. Added a normative audit-event refinement for the module-originated cryptographic and data-protection SFRs, with secret-safe audit content rules. Published the coordinated PQC and ML-KEM role model as Version 0.5.
...
* [#SD]#[SD]# Supporting Document - Evaluation Activities for DBMS Cryptographic Functions Module, Version 0.5.
...
* [#DBMS_Cloud_MOD]#[DBMS_Cloud_MOD]# collaborative PP-Module for DBMS in the Cloud, Version 0.5.
* [#DBMS_MOD_DBAAS]#[DBMS_MOD_DBAAS]# collaborative PP-Module for Database-as-a-Service (DBaaS), Version 0.5, DBMS-iTC.
...
+
This configuration is required when the DBMS DBaaS Module [<<DBMS_MOD_DBAAS>>] is claimed. The DBaaS Module mandates the inclusion of this Crypto Module to provide Data-in-Transit protections using TLS, including HTTPS over TLS where applicable, and Data-at-Rest encryption protections. See the DBMS DBaaS Module for full PP-Configuration requirements.

The DBMS Cloud Module and the DBMS DBaaS Module are mutually exclusive: they are never combined in a single PP-Configuration. A TOE claims one or the other according to who holds administrative authority over the DBMS software lifecycle.
...
----
                ...
                |                         |
      +-------------------+     +-------------------+
      | DBMS Cloud Module |     | DBMS DBaaS Module |
      |    (Optional)     |     |    (Optional)     |
      +-------------------+     +-------------------+
----

*Application Note:* When the DBMS Cloud Module or the DBMS DBaaS Module is included in the PP-Configuration, this module is a mandatory dependency of that module. When neither is included, this module may be claimed independently against the cPP_DBMS Base PP.
...
The Enterprise Enhanced use case addresses only cryptographic functions within the scope of this module, principally symmetric data protection, key derivation and protection, hashing, one-time values, random bit generation, signature verification, and non-protocol key establishment. Protocol and certificate requirements are supplied by the applicable Functional Packages or other Requirements Documents, including post-quantum key establishment within protocols. Within this module's scope, the ML-DSA and SP 800-208 rows of `FCS_COP.1/SigVer` and the role-specific ML-KEM completions of `FCS_COP.1/KeyEncap` and `FCS_CKM.1/KEM` carry the applicable post-quantum selections. Selection of this use case therefore demonstrates alignment of the in-scope operations; it does not by itself assert FIPS 140-3 validation, NIAP approval, CNSA 2.0 conformance for an entire product, or eligibility for any product list.
...
. *Selection-Based SFRs*: The ST shall include SFRs from <<Security Functional Requirements (Selection-Based)>> when selections in mandatory or other selection-based SFRs trigger their inclusion. `FCS_CKM.1/SKG` is required when the TOE directly generates DEKs or internally generates MEKs. `FCS_CKM.1/KEM` is required when the TOE generates an ML-KEM key pair. `FCS_COP.1/KeyEncap` is required when the TOE performs non-protocol encapsulation or decapsulation for a selected Master Key origin or supplementary transferred-key protection. `FDP_ITC_EXT.1` is required when "Imported from External Entity", "Externally Managed", or "Established through Key Encapsulation" is selected in `FCS_CKM_EXT.1.1`.
...
The operational environment protects the storage media, hardware security services, and platform facilities on which the TOE relies for cryptographic key storage from physical access and administrative bypass outside the TOE boundary. The environment ensures that:
+
* Storage and hardware services allocated to the operational environment cannot be accessed or reconfigured without proper authorization;
* Access outside the TOE's interfaces is protected by authentication and access-control mechanisms independent of the TOE;
* The integrity of the supporting storage and hardware services is protected against unauthorized modification.
+
This assumption does not replace the TOE's responsibility in `FCS_CKM_EXT.1.2` to protect Master Key material present within the TOE boundary.
...
+
*Application Note:* This objective applies when "Imported from External Entity", "Externally Managed", or "Established through Key Encapsulation" is selected in `FCS_CKM_EXT.1.1`. For a TOE that generates its Master Keys internally, no external key interface exists; the threat T.KEY_IMPORT_INTERCEPTION is vacuously countered. This objective and FDP_ITC_EXT.1 apply together for every external Key Origin, including authentication of the external entity participating in KEM establishment.
...
The operational environment shall protect storage media, hardware security services, and platform facilities allocated outside the TOE boundary from unauthorized physical access, administrative bypass, extraction, and modification. These protections may include hardware security modules (HSMs), trusted platform modules (TPMs), secure enclaves, encrypted filesystems, and platform access controls. This objective supports, but does not substitute for, the TOE protection selected in `FCS_CKM_EXT.1.2` for Master Key material present within the TOE boundary.
...
| T.KEY_IMPORT_INTERCEPTION
| O.TRUSTED_KEY_IMPORT
| When "Imported from External Entity", "Externally Managed", or "Established through Key Encapsulation" is selected in `FCS_CKM_EXT.1.1`, establishing a trusted channel with confidentiality, integrity, and authentication controls (O.TRUSTED_KEY_IMPORT) prevents a malicious actor from intercepting or substituting key material, ML-KEM public keys, or KEM ciphertexts during BYOK import, ongoing external key-management exchanges, or Master Key establishment.
...
| A.KEY_STORE_SECURITY
| OE.KEY_STORE_PROTECTION
| The operational environment objective OE.KEY_STORE_PROTECTION directly requires secure physical and logical protection for storage media, hardware security services, and platform facilities outside the TOE boundary, which is the property assumed in A.KEY_STORE_SECURITY. It complements, and does not replace, the TOE's selected `FCS_CKM_EXT.1.2` protection for Master Key material within the TOE boundary.
...
| O.MASTER_KEY_MANAGEMENT
| FCS_CKM_EXT.1 +
  *Crypto Catalogue: FCS_CKM.1/SKG and FCS_RBG.1 when MEKs are generated internally; FCS_CKM.1/KEM, FCS_COP.1/KeyEncap, FCS_CKM.5, and conditional FCS_RBG.1 for a KEM-established origin; FCS_CKM.6; and selected DEK protection operations*
| FCS_CKM_EXT.1 manages the full MEK lifecycle including generation, establishment, import, DEK protection, and destruction. Internally generated MEKs require `FCS_CKM.1/SKG` and `FCS_RBG.1`. A KEM-established origin uses the TOE-role operations selected in `FCS_COP.1/KeyEncap`, `FCS_CKM.1/KEM` only when the TOE generates the ML-KEM key pair, `FCS_RBG.1` only for TOE KeyGen or encapsulation, and `FCS_CKM.5` to derive the MEK from the shared secret. Protected DEK storage requires the selected `FCS_COP.1/KeyWrap`, `FCS_COP.1/SKC`, or `FCS_COP.1/AEAD` claim, and `FCS_CKM.6` specifies destruction of in-scope secret values.
...
| FCS_CKM.1/SKG (when direct symmetric key generation is claimed) +
  FCS_CKM.1/KEM (when TOE ML-KEM key-pair generation is claimed) +
  FCS_CKM.5 or FCS_CKM_EXT.8 (when DEKs are derived) +
  FDP_DAR_EXT.1 +
  FDP_ITC_EXT.1 +
  *Crypto Catalogue: All claimed components*
| Each in-scope algorithm is specified by a Catalogue-derived or explicitly anticipated component reproduced in this module. `FCS_CKM.1/SKG` requires the consumed RBG components when direct symmetric key generation is selected; `FCS_CKM.1/KEM` and `FCS_COP.1/KeyEncap` separate ML-KEM KeyGen from the TOE's selected encapsulation or decapsulation role; derived DEKs and KEM-established MEKs require the applicable consumed key-derivation component; and FDP_DAR_EXT.1 requires a consumed symmetric or authenticated-encryption operation. FDP_ITC_EXT.1 requires a mutual TLS channel specified by the TLS Functional Package when an external Key Origin is selected.
...
* T.KEY_IMPORT_INTERCEPTION: Addressed by O.TRUSTED_KEY_IMPORT when "Imported from External Entity", "Externally Managed", or "Established through Key Encapsulation" is selected in FCS_CKM_EXT.1.1
...
* O.MASTER_KEY_MANAGEMENT: FCS_CKM_EXT.1 and the consumed or anticipated components selected for internal generation, KEM establishment, and DEK protection
* O.TRUSTED_KEY_IMPORT: FDP_ITC_EXT.1 when "Imported from External Entity", "Externally Managed", or "Established through Key Encapsulation" is selected in FCS_CKM_EXT.1.1
...
* *Key Origin*: [selection: Generated Internally - The TSF shall generate the Master Key using the consumed `FCS_CKM.1/SKG` and `FCS_RBG.1` components and store it securely within the TOE boundary; Imported from External Entity - The TSF shall import the Master Key from an external IT entity via the trusted channel defined in FDP_ITC_EXT.1, after which the TSF is the authoritative holder of the Master Key; Externally Managed - The TSF shall obtain use of the Master Key from an external key management service via the trusted channel defined in FDP_ITC_EXT.1, by retrieving the Master Key for transient use or by invoking Master Key operations by reference; the external service remains the authoritative store, and the TSF shall not persistently store plaintext Master Key material within the TOE boundary; Established through Key Encapsulation - The TSF shall establish a shared secret with an external key management entity using the TOE-role operations selected in `FCS_COP.1/KeyEncap`, derive the Master Key from that shared secret using `FCS_CKM.5`, and store the established Master Key securely within the TOE boundary.]

*FCS_CKM_EXT.1.2* The TSF shall restrict use of every Master Key present within the TOE boundary to authorized TSF functions and roles, and shall protect persistent Master Key material against disclosure and modification using [selection: cryptographic protection claimed as [selection: `FCS_COP.1/KeyWrap`, `FCS_COP.1/AEAD`], non-exportable hardware-backed key storage identified in the ST and TSS, no persistent Master Key material within the TOE boundary]. The TSF shall protect stored DEKs under the Master Key using [selection: key wrapping claimed as `FCS_COP.1/KeyWrap`, symmetric key encryption claimed as `FCS_COP.1/SKC`, authenticated encryption claimed as `FCS_COP.1/AEAD`].
...
*Application Note:* This requirement models the DBMS-specific behavior of managing a root key (e.g., TDE Master Key). Four Key Origins are defined: internal generation; one-time import in which custody of the Master Key transfers to the TOE (e.g., BYOK); external management, in which an external key management service - for example, a KMIP-conformant key manager [<<KMIP>>] - remains the authoritative store and the TOE retrieves the Master Key for transient use or invokes its operations by reference; and establishment through ML-KEM, in which the TOE and an external entity establish a shared secret and the TOE derives the Master Key from it. Every external origin includes `FDP_ITC_EXT.1`; the key-management message protocol is carried over the trusted channel and is not itself evaluated. For a KEM-established origin, the ST selects encapsulation, decapsulation, or both according to the TOE role; includes `FCS_CKM.1/KEM` only when the TOE generates the ML-KEM key pair; includes `FCS_RBG.1` when the TOE performs KeyGen or encapsulation; and identifies provisioned keys and externally performed operations without claiming them as TSF operations. Protocol-level ML-KEM, including hybrid TLS, remains governed by the applicable Functional Package.
+
The first selection in `FCS_CKM_EXT.1.2` makes the TOE's Master Key protection claim explicit. Cryptographic protection requires the corresponding Catalogue component. Non-exportable hardware-backed storage applies only when the hardware interface and the non-exportability enforcement are within the evaluated TOE boundary; a hardware service allocated solely to the operational environment cannot satisfy this selection. The no-persistent-material selection is appropriate for by-reference or transient-use designs and does not excuse plaintext persistence elsewhere in the TOE. Multiple selections may be made when the TOE supports more than one Master Key storage path. `OE.KEY_STORE_PROTECTION` covers supporting storage and hardware outside the TOE boundary and does not substitute for this TSF behavior.
+
The DEK protection selection in `FCS_CKM_EXT.1.2` remains symmetric: the KEM establishes the MEK, and the selected `FCS_COP.1/KeyWrap`, `FCS_COP.1/SKC`, or `FCS_COP.1/AEAD` operation protects stored DEKs under it. Where an external service performs operations by reference, the ST claims FCS components only for TSF-implemented operations. The symmetric key encryption option provides confidentiality but not modification detection; the Enterprise Enhanced use case does not permit it. For the "Externally Managed" origin, FCS_CKM_EXT.1.3 applies to transient TSF copies while authoritative destruction is performed externally. If a selected operation requires one-time values, the ST shall include `FCS_OTV_EXT.1`.
...
| FCS_COP.1/KeyEncap, FCS_CKM.5, FCS_CKM.1/KEM, and FCS_RBG.1 (conditional)
| When "Established through Key Encapsulation" is selected, the ST includes `FCS_COP.1/KeyEncap` for the TOE role and `FCS_CKM.5` for Master Key derivation. It additionally includes `FCS_CKM.1/KEM` and `FCS_RBG.1` when the TOE generates the ML-KEM key pair, and `FCS_RBG.1` when the TOE performs encapsulation.

| FCS_COP.1/KeyWrap or FCS_COP.1/AEAD Master Key Protection Operation (conditional); FCS_COP.1/KeyWrap, FCS_COP.1/SKC, or FCS_COP.1/AEAD DEK Protection Operation
| Cryptographic protection of persistent Master Key material is satisfied by the selected `FCS_COP.1/KeyWrap` or `FCS_COP.1/AEAD` component. DEK protection is satisfied by the selected `FCS_COP.1/KeyWrap`, `FCS_COP.1/SKC`, or `FCS_COP.1/AEAD` component. `FCS_OTV_EXT.1` is required when a selected operation has an IV, nonce, tweak, or other one-time-value dependency.

| FDP_ITC_EXT.1 Trusted Channel for Key Import, External Key Management, and Key Establishment (conditional)
| Satisfied by FDP_ITC_EXT.1 in this PP-Module. Required when "Imported from External Entity", "Externally Managed", or "Established through Key Encapsulation" is selected in FCS_CKM_EXT.1.1.
|===
...
The Catalogue-derived selection-based cryptographic requirements are reproduced in <<Consumed Cryptographic Catalogue Components>>. `FCS_CKM.1/SKG` is included when the TOE directly generates DEKs or internally generates MEKs. The anticipated `FCS_CKM.1/KEM` iteration is included only when the TOE generates an ML-KEM key pair. `FCS_CKM.5` or `FCS_CKM_EXT.8` is included when the TOE derives in-scope keys, including `FCS_CKM.5` for the Master Key derived from a KEM shared secret. The supporting cryptographic operation, one-time-value, random-bit-generation, seeding, failure, and self-test components are included as triggered by those selections.

=== FDP: User Data Protection

==== FDP_ITC_EXT.1 Trusted Channel for Key Import, External Key Management, and Key Establishment

*FDP_ITC_EXT.1.1* The TSF shall be capable of establishing a trusted channel between itself and an external key management entity for the purpose of importing cryptographic keys, communicating with an external key management service, or exchanging authenticated values used to establish keying material through key encapsulation.
...
*FDP_ITC_EXT.1.4* Keying material transferred over the trusted channel shall be protected by [selection: the trusted channel alone, supplementary end-to-end key wrapping claimed as `FCS_COP.1/KeyWrap`, supplementary end-to-end protection using a key-encryption key established through `FCS_COP.1/KeyEncap` and derived from the ML-KEM shared secret using `FCS_CKM.5`, with the transferred keying material protected using [selection: key wrapping claimed as `FCS_COP.1/KeyWrap`, authenticated encryption claimed as `FCS_COP.1/AEAD`]].

*Application Note:* This SFR is required when "Imported from External Entity", "Externally Managed", or "Established through Key Encapsulation" is selected in `FCS_CKM_EXT.1.1`. It covers one-time key import (e.g., BYOK), ongoing external key-management integration (e.g., a KMIP-conformant key manager [<<KMIP>>]), and the authenticated exchange used for a KEM-established Master Key. The key-management protocol messages are carried over the mutual TLS channel, and the protocol itself is not evaluated. Mutual TLS is mandatory for every external Key Origin.

FDP_ITC_EXT.1.4 distinguishes key wrapping from key establishment. ML-KEM does not directly encapsulate a pre-existing MEK or DEK. When supplementary KEM protection is selected, `FCS_COP.1/KeyEncap` establishes a shared secret, `FCS_CKM.5` derives a key-encryption key with the context and purpose identified in the ST, and `FCS_COP.1/KeyWrap` or `FCS_COP.1/AEAD` protects the transferred keying material. The ST selects encapsulation, decapsulation, or both according to the TOE role and claims `FCS_CKM.1/KEM` only if the TOE generates the ML-KEM key pair. The trusted channel remains mandatory, and "the trusted channel alone" is the expected selection for TOEs without supplementary end-to-end protection.
...
| FCS_COP.1/KeyWrap; or FCS_COP.1/KeyEncap, FCS_CKM.5, and FCS_COP.1/KeyWrap or FCS_COP.1/AEAD (conditional)
| Direct supplementary wrapping includes `FCS_COP.1/KeyWrap`. The KEM-based selection includes `FCS_COP.1/KeyEncap` for the TOE role, `FCS_CKM.5` to derive the key-encryption key, and `FCS_COP.1/KeyWrap` or `FCS_COP.1/AEAD` to protect the transferred keying material. `FCS_CKM.1/KEM` and `FCS_RBG.1` are additionally included when the TOE performs KeyGen; `FCS_RBG.1` is included when it performs encapsulation.
|===
...
| FCS_CKM.1/KEM
| FCS_CKM.6; FCS_RBG.1
| Anticipated post-Version 1.0 key-generation iteration included only when the TOE performs ML-KEM.KeyGen; resolved by the consumed key-destruction and RBG component set.

| FCS_CKM.5
| [`FCS_CKM.2` or `FCS_COP.1`] (CCDB-018 Section 3.6); `FCS_CKM.6`; [`FCS_COP.1/CMAC`, `FCS_COP.1/Hash`, `FCS_COP.1/KeyedHash`, `FCS_COP.1/SKC`, or `FCS_COP.1/AEAD`]
| Included when the TOE derives an in-scope key from non-password input, including the Master Key derived from a KEM shared secret. Resolved by the consumed key-destruction and derivation-primitive components; the [`FCS_CKM.2` or `FCS_COP.1`] key-utilization dependency is resolved by the `FCS_COP.1/KeyWrap` or `FCS_COP.1/AEAD` component that consumes the derived Master Key.

| FCS_CKM_EXT.1
| FCS_CKM.6 (CCDB-018 Section 3.7); FCS_CKM.1/SKG and FCS_RBG.1 (when MEKs are generated internally); FCS_COP.1/KeyEncap and FCS_CKM.5 for a KEM-established origin, with FCS_CKM.1/KEM and FCS_RBG.1 as triggered by the TOE role; selected `FCS_COP.1/KeyWrap` or `FCS_COP.1/AEAD` for cryptographic protection of persistent MEK material; selected `FCS_COP.1/KeyWrap`, `FCS_COP.1/SKC`, or `FCS_COP.1/AEAD` for DEK protection; `FCS_OTV_EXT.1` when required; FDP_ITC_EXT.1 (when an external origin is selected)
| Crypto Catalogue and anticipated post-Version 1.0 components for selected key-generation, establishment, derivation, and protection claims; FDP_ITC_EXT.1 for every external Key Origin
...
When this PP-Module is claimed, Table 4: Auditable Events of the Base PP is refined to add the following entries. This refinement makes the audit records used by this module's Evaluation Activities normatively required. Auditable events for Catalogue-derived components remain as specified by the Cryptographic Catalogue; this table adds the DBMS-specific lifecycle and integration events of the SFRs originated by this module.

Audit records shall not include plaintext keys, raw shared secrets, passwords, private-key material, session secrets, or protected user data. An identifier, fingerprint, key reference, certificate subject, or other non-secret correlation value may be recorded instead.

.Auditable Events for Cryptographic Functions Module SFRs
[%header,cols="2,4,4"]
|===
...
|FCS_CKM_EXT.1
|Master Key generation, import, activation through an external key-management reference, establishment through key encapsulation, rotation, destruction, use-authorization failure, and failure to obtain or activate a Master Key
|Master Key identifier or non-secret reference; selected Key Origin; operation; result; external entity identifier where applicable; selected MEK-protection model; no key material or raw shared secret

|FDP_DAR_EXT.1
|Enabling or disabling data-at-rest protection; change to the encryption strategy, protected storage scope, or documented exclusion; encryption or decryption failure
|Affected database, storage-object category, or protected object identifier; selected strategy; operation; result or failure reason; no plaintext user data

|FDP_ITC_EXT.1
|Establishment or failure of the trusted channel to an external key-management entity; successful or failed key import, external key-use request, or key-establishment operation; peer-authentication or certificate-validation failure; rejection of invalid KEM input or supplementary protected keying material
|External entity identifier; channel endpoint; operation and TOE role; non-secret key identifier or reference; protocol and certificate subject or trust-anchor identifier where applicable; result or failure reason; no transferred keying material, private key, shared secret, or derived KEK

|FDP_DIT_EXT.1
|Establishment or failure of a protected external channel; enabling or disabling data-in-transit protection; endpoint-authentication or certificate-validation failure
|External endpoint or interface; TOE client/server role; claimed protocol; certificate subject or trust-anchor identifier where applicable; result or failure reason; no credential, session secret, or protected payload
|===
...
| "Established through Key Encapsulation" selected for MEK origin in FCS_CKM_EXT.1.1
| `FCS_COP.1/KeyEncap` for the TOE role and `FCS_CKM.5` for MEK derivation; `FCS_CKM.1/KEM` and `FCS_RBG.1` when the TOE performs KeyGen; `FCS_RBG.1` when the TOE performs encapsulation
| FCS_CKM_EXT.1
...
| Key wrapping selected for Master Key cryptographic protection or DEK protection in FCS_CKM_EXT.1.2
| `FCS_COP.1/KeyWrap`, plus `FCS_OTV_EXT.1` when required by the selected Catalogue operation
| FCS_CKM_EXT.1

| Authenticated encryption selected for Master Key cryptographic protection or DEK protection in FCS_CKM_EXT.1.2
| `FCS_COP.1/AEAD`, plus `FCS_OTV_EXT.1` when required by the selected mode
| FCS_CKM_EXT.1

| Symmetric key encryption selected for DEK protection in FCS_CKM_EXT.1.2
| `FCS_COP.1/SKC`, plus `FCS_OTV_EXT.1` when required by the selected mode
| FCS_CKM_EXT.1
...
| "Imported from External Entity", "Externally Managed", or "Established through Key Encapsulation" selected in FCS_CKM_EXT.1.1
...
| KEM-based supplementary end-to-end protection selected in FDP_ITC_EXT.1.4
| `FCS_COP.1/KeyEncap`, `FCS_CKM.5`, and `FCS_COP.1/KeyWrap` or `FCS_COP.1/AEAD`; `FCS_CKM.1/KEM` and `FCS_RBG.1` when the TOE performs KeyGen; `FCS_RBG.1` when the TOE performs encapsulation
| FDP_ITC_EXT.1
...
[#FCS_CKM_1_KEM]
==== FCS_CKM.1/KEM Cryptographic Key Generation - ML-KEM Key Pair

*Source and modification:* Anticipates the CCDB-018 post-Version 1.0 key-generation component for ML-KEM. This iteration is included only when the TOE performs FIPS PUB 203 ML-KEM.KeyGen; a provisioned or externally generated key pair does not trigger it.

Hierarchical to: No other components.

Dependencies: [`FCS_CKM.2` or `FCS_COP.1`]; `FCS_CKM.6`; `FCS_RBG.1`.

*FCS_CKM.1.1/KEM* The TSF shall generate ML-KEM encapsulation and decapsulation key pairs in accordance with the specified cryptographic key generation algorithm [ML-KEM.KeyGen] and cryptographic algorithm parameters [selection: ML-KEM-512, ML-KEM-768, ML-KEM-1024] that meet the following standard: [FIPS PUB 203].

*Application Note:* This component claims only TOE-performed ML-KEM key-pair generation. `FCS_COP.1/KeyEncap` separately selects encapsulation, decapsulation, or both according to the TOE role. The ST identifies whether each encapsulation or decapsulation key is TOE-generated, provisioned, imported, or held by an external service; it shall not claim `FCS_CKM.1/KEM` for generation outside the TOE boundary. The decapsulation key, KeyGen seed, and any other secret representation are subject to `FCS_CKM.6`. Algorithm validation evidence shall map the KeyGen capability separately from encapsulation and decapsulation.
...
*Application Note:* In KDF-MAC-2S, if a CMAC is selected in the MAC step, select AES-128-CMAC or Camellia-128-CMAC in the KDF step and select 128 as the output key size. If HMAC is selected in the MAC step, select the same HMAC in the KDF. The respective `FCS_COP.1` component must be included for each primitive selected in the key derivation algorithm. When the input is an ML-KEM shared secret established by `FCS_COP.1/KeyEncap`, select a shared-secret completion such as KDF-HASH, KDF-MAC-1S, or KDF-MAC-2S and identify fixed information or context that binds the output to the TOE, peer, key purpose, and applicable transaction or protected-key record. The derived output is the MEK or key-encryption key; the raw ML-KEM shared secret shall not be reused for another purpose.
...
*Source and modification:* Anticipates the CCDB-018 post-Version 1.0 key-encapsulation component (see the Application Note). This component is included when "Established through Key Encapsulation" is selected in `FCS_CKM_EXT.1.1` or when KEM-based supplementary protection is selected in `FDP_ITC_EXT.1.4`. Key encapsulation within a cryptographic protocol (for example, hybrid key establishment in TLS) remains governed by the applicable Functional Package and is not claimed through this component.

Hierarchical to: No other components.

Dependencies: [`FDP_ITC.1`, `FDP_ITC.2`, `FCS_CKM.1/KEM`, `FCS_CKM_EXT.1`, or no other component]; `FCS_CKM.6`; `FCS_RBG.1` when encapsulation is selected; `FCS_CKM.1/KEM` and `FCS_RBG.1` when the TOE generates the ML-KEM key pair.

*FCS_COP.1.1/KeyEncap* The TSF shall perform [selection: encapsulation, decapsulation, encapsulation and decapsulation] in accordance with a specified cryptographic algorithm [selection: cryptographic algorithm] and cryptographic algorithm parameters [selection: cryptographic algorithm parameters] that meet the following: [selection: list of standards].
...
*Application Note:* This component anticipates the Catalogue's post-Version 1.0 key-encapsulation development: FIPS PUB 203 is final, CAVP validates ML-KEM implementations, and the transitional evidence mechanism of the Crypto SD applies until the corresponding Catalogue entry and Evaluation Methods are available. The ST selects the operations actually performed by the TOE. Encapsulation uses an encapsulation public key and fresh randomness and therefore triggers `FCS_RBG.1`; decapsulation uses the TOE's decapsulation key and does not by itself trigger a fresh-randomness dependency. TOE-performed ML-KEM.KeyGen is claimed separately through `FCS_CKM.1/KEM`; provisioned or externally generated keys do not trigger that component. The decapsulation key, KeyGen seed, shared secret, and any derived MEK or key-encryption key are secret values subject to `FCS_CKM.6`; the public encapsulation key is not secret but its authenticity and association with the intended recipient shall be described in the TSS. For a KEM-established Master Key, `FCS_CKM.5` derives the MEK from the shared secret. For supplementary protection of a pre-existing transferred key, `FCS_CKM.5` derives a key-encryption key and `FCS_COP.1/KeyWrap` or `FCS_COP.1/AEAD` protects the transferred key. The "or no other component" choice in the key-source dependency is a deliberate DBMS-iTC relaxation of the standard `FCS_COP.1` key-source dependency, added to accommodate a provisioned decapsulation key that the TOE did not itself generate, import, or derive; the protection of such a provisioned decapsulation key is addressed in the TSS and by `FCS_CKM.6`. Protocol-level KEMs remain Functional Package-governed.
...
Dependencies: `FCS_RBG.1`; [`FCS_COP.1/KeyedHash`, `FCS_COP.1/SKC`, `FCS_CKM.5`, `FCS_CKM_EXT.8`, `FCS_COP.1/CMAC`, `FCS_COP.1/KeyWrap`, or `FCS_COP.1/AEAD`].
...
|`FCS_COP.1/KeyEncap`
|When key encapsulation is selected for a KEM-established Master Key origin or in `FDP_ITC_EXT.1.4`, select ML-KEM with the ML-KEM-1024 parameter set and only the operations performed by the TOE. When the TOE performs KeyGen, complete `FCS_CKM.1/KEM` with ML-KEM-1024.
...
a) Selection of Master Key origin (internal generation, external import, external management, or establishment through key encapsulation).
...
c) Basic: Activation of a Master Key through an external key-management reference.
d) Basic: Establishment of a Master Key through key encapsulation.
e) Basic: Destruction of a Master Key.
f) Basic: Failure to obtain, authorize, or activate a Master Key.
g) Detailed: Key rotation operations and changes to the selected Master Key protection model.

===== FCS_CKM_EXT.1 Cryptographic Key Management (Master Keys)

Hierarchical to: No other components.

Dependencies: `FCS_CKM.6` Timing and Event of Cryptographic Key Destruction (CCDB-018 Section 3.7); `FCS_CKM.1/SKG` and `FCS_RBG.1` when MEKs are generated internally; `FCS_COP.1/KeyEncap` and `FCS_CKM.5` when the MEK is established through key encapsulation, with `FCS_CKM.1/KEM` and `FCS_RBG.1` as triggered by the TOE role; `FCS_COP.1/KeyWrap` or `FCS_COP.1/AEAD` when selected for cryptographic protection of persistent MEK material; `FCS_COP.1/KeyWrap`, `FCS_COP.1/SKC`, or `FCS_COP.1/AEAD` for DEK protection as selected; `FCS_OTV_EXT.1` when required by a selected operation; `FDP_ITC_EXT.1` Trusted Channel for Key Import, External Key Management, and Key Establishment (this module, conditional when an external Key Origin is selected).
...
==== FDP_ITC_EXT: Trusted Channel for Key Import, External Key Management, and Key Establishment

===== Family Behaviour

This family defines requirements for the TSF to establish a mutually authenticated TLS trusted channel to an external key management entity, for the purpose of securely importing cryptographic key material, communicating with an external key management service that remains the authoritative store for the Master Key (e.g., a KMIP-conformant key manager), or authenticating the exchange used to establish a Master Key through key encapsulation. It complements FCS_CKM_EXT.1 by defining the communication channel properties required for every external Key Origin. It is distinct from the general trusted channel families (FTP_ITC, FTP_TRP) in that it is scoped to key management operations and explicitly defines mutual authentication requirements in that context.

===== Component levelling

FDP_ITC_EXT.1 Trusted Channel for Key Import, External Key Management, and Key Establishment requires the TSF to establish a trusted channel that provides confidentiality, integrity, and authentication for key import, external key-management, and key-establishment operations.
...
*FDP_DIT_EXT.1.1* The TSF shall protect data in transit between the TOE and external entities from unauthorized disclosure and modification using TLS as claimed from the Functional Package for TLS v2.1 and [selection: no other cryptographic protocol, [assignment: other cryptographic protocol claimed from an applicable Functional Package]].

*FDP_DIT_EXT.1.2* The TSF shall authenticate the endpoints of the protected channel using X.509 certificates claimed from the Functional Package for X.509 Certificates, where required by the selected protocol.
...
|FCS
|FCS_CKM.1/KEM Cryptographic Key Generation (ML-KEM Key Pair)
|Selection-Based; included only when the TOE performs ML-KEM.KeyGen
...
|FCS
|FCS_COP.1/KeyEncap Cryptographic Operation - Key Encapsulation
|Selection-Based; included for a KEM-established Master Key origin or KEM-based supplementary transferred-key protection, with operations selected according to the TOE role
...
|FCS_CKM.1/KEM
|[`FCS_CKM.2` or `FCS_COP.1`]; `FCS_CKM.6`; `FCS_RBG.1`
|Included only for TOE-performed ML-KEM.KeyGen and resolved by the consumed key-destruction and RBG component set. The [`FCS_CKM.2` or `FCS_COP.1`] key-utilization dependency is resolved by `FCS_COP.1/KeyEncap`, which consumes the generated ML-KEM key pair. Provisioned or externally generated ML-KEM key pairs do not trigger this component.

|FCS_CKM.5 / FCS_CKM_EXT.8
|[`FCS_CKM.2` or `FCS_COP.1`]; `FCS_CKM.6`; applicable `FCS_COP.1` primitive; `FCS_OTV_EXT.1` for password-based derivation
|Resolved by the consumed key-destruction, primitive, and one-time-value components selected for the derivation path. The [`FCS_CKM.2` or `FCS_COP.1`] key-utilization dependency is resolved by the `FCS_COP.1/KeyWrap` or `FCS_COP.1/AEAD` component that consumes the derived Master Key.

|FCS_CKM.6
|A component that creates, imports, accesses, derives, or agrees the keys being destroyed
|Resolved by `FCS_CKM_EXT.1`, `FCS_CKM.1/SKG`, `FCS_CKM.1/KEM`, `FCS_CKM.5`, or `FCS_CKM_EXT.8`, as applicable. For external Master Key origins, `FCS_CKM_EXT.1` together with `FDP_ITC_EXT.1` provides the key-sourcing or establishment function that the Catalogue's `FDP_ITC.1`/`FDP_ITC.2` alternatives would otherwise provide; this substitution is justified because those components are the DBMS-specific instantiations of the same dependency purpose.
...
|FCS_COP.1/KeyEncap
|Optional key source dependency; `FCS_CKM.6`; `FCS_RBG.1` when encapsulation is selected; `FCS_CKM.1/KEM` and `FCS_RBG.1` when the TOE performs KeyGen
|Included through the `FCS_CKM_EXT.1.1` KEM-established origin or the KEM-based `FDP_ITC_EXT.1.4` selection. The ST selects encapsulation, decapsulation, or both according to the TOE role. `FCS_CKM.1/KEM` is claimed separately for TOE KeyGen; decapsulation alone does not trigger fresh randomness.
...
FCS_COP.1/KeyEncap and FCS_CKM.5 (when the MEK is established through key encapsulation), with FCS_CKM.1/KEM and FCS_RBG.1 as triggered by the TOE role +
...
| FCS_CKM_EXT.1 (when an external Key Origin is selected)
| FDP_ITC_EXT.1
| FDP_ITC_EXT.1 (this module, selection-based when an external Key Origin is selected)
...
| FCS_CKM.1/KEM
| [FCS_CKM.2 or FCS_COP.1]; FCS_CKM.6; FCS_RBG.1
| Anticipated `FCS_COP.1/KeyEncap` for the [FCS_CKM.2 or FCS_COP.1] key-utilization dependency, consuming the generated ML-KEM key pair; consumed `FCS_CKM.6`, `FCS_RBG.1`, and the selected seeding, primitive, failure, and self-test dependencies when the TOE performs ML-KEM.KeyGen

| FCS_CKM_EXT.1
| FCS_CKM.6; FCS_CKM.1/SKG and FCS_RBG.1 when MEKs are generated internally; FCS_COP.1/KeyEncap and FCS_CKM.5 for a KEM-established origin, with FCS_CKM.1/KEM and FCS_RBG.1 as triggered by the TOE role; selected DEK protection operation
| Consumed `FCS_CKM.6`; consumed `FCS_CKM.1/SKG` and `FCS_RBG.1` when MEKs are generated internally; anticipated `FCS_COP.1/KeyEncap` and `FCS_CKM.1/KEM` plus consumed `FCS_CKM.5` and conditional `FCS_RBG.1` for a KEM-established origin; selected consumed `FCS_COP.1/KeyWrap`, `FCS_COP.1/SKC`, or `FCS_COP.1/AEAD`
...
This PP-Module ordinarily consumes and reproduces applicable components from the Common Criteria Cryptographic Catalogue [<<Crypto_Catalog>>], then adds only the DBMS-specific applicability and integration requirements needed for the TOE type. The explicitly identified `FCS_CKM.1/KEM` and ML-KEM row of `FCS_COP.1/KeyEncap` anticipate post-Version 1.0 Catalogue content because FIPS PUB 203 is final; their provenance, transitional evaluation treatment, and replacement by scheme-recognized Catalogue content are stated explicitly.

. *Separation of Concerns:* The Catalogue is maintained through the Common Criteria cryptographic standards process. Retaining its component identifiers, operations, dependencies, ECDs, and evaluation methods prevents this PP-Module from presenting published Catalogue material as DBMS-iTC-originated content. Anticipated ML-KEM content is clearly marked and shall yield to the applicable scheme-recognized Catalogue component and Evaluation Methods when published.
