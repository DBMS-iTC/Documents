...
The Enterprise Enhanced use case addresses only cryptographic functions within the scope of this module, principally symmetric data protection, key derivation and protection, hashing, one-time values, random bit generation, signature verification, and the optional key-encapsulation component. Protocol and certificate requirements are supplied by the applicable Functional Packages or other Requirements Documents, including post-quantum key establishment within protocols. Within this module's scope, the ML-DSA and SP 800-208 rows of `FCS_COP.1/SigVer` and the ML-KEM completions of the optional `FCS_COP.1/KeyEncap` carry the applicable post-quantum selections. Selection of this use case therefore demonstrates alignment of the in-scope operations; it does not by itself assert FIPS 140-3 validation, NIAP approval, CNSA 2.0 conformance for an entire product, or eligibility for any product list.
...
|`FCS_COP.1/KeyEncap`
|When the optional key-encapsulation component is included, select ML-KEM with the ML-KEM-1024 parameter set.
...
This appendix defines one optional component. Under the Exact Conformance statement of this PP-Module, an ST may include it when the TOE provides the corresponding function; its omission does not affect conformance.

[#FCS_COP_1_KeyEncap]
=== FCS_COP.1/KeyEncap Cryptographic Operation - Key Encapsulation

*Source and modification:* Anticipates the CCDB-018 post-Version 1.0 key-encapsulation component (see the Application Note). This optional component may be included when the TSF performs KEM-based establishment or transport of symmetric keying material for an in-scope DBMS function — for example, protecting keying material delivered to or received from an external entity. Key encapsulation within a cryptographic protocol (for example, hybrid key establishment in TLS) remains governed by the applicable Functional Package and is not claimed through this component.
...
|FCS_COP.1/KeyEncap Cryptographic Operation - Key Encapsulation
|Optional; defined in the Optional Requirements appendix
...
|FCS_COP.1/KeyEncap (optional)
|Optional key source dependency; `FCS_CKM.6`; `FCS_RBG.1`
|Resolved by the consumed key-destruction and RBG components; the encapsulation key pair is generated under this component or imported per the selected dependency.
