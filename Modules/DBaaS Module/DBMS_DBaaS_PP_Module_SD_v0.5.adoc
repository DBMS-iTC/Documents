...
For `FMT_MOF.1/DBaaS-Disable`, the evaluator shall confirm, from the TSS and the functional specification, that the interfaces which manage audit generation and data-at-rest encryption are enumerated, and that none of those interfaces, in any role, disables audit generation or data-at-rest encryption, consistent with the component's refinement and aligned to the "no interface" idiom of `FCS_CKM_DBAAS_EXT.1.2` and `FDP_BKP_EXT.1.1`.
...
. Confirm, from the TSS and the functional specification, the enumeration of interfaces that manage audit generation, and that none is documented to disable audit generation for TOE-required audit events in any role.
. Attempt to disable audit generation for TOE-required audit events through tenant-administrator interfaces (the tenant data plane V1 and the tenant service control plane V2) and, through a provider-supported demonstration at the provider-internal vantage (V4) under the Evaluator Access Strategy (<<_evaluator_access_strategy>>) recorded in the ETR, through service-provider-administrator interfaces.
. Verify that no interface, in any role, performs the disable.
...
. Confirm, from the TSS and the functional specification, the enumeration of interfaces that manage data-at-rest encryption, and that none is documented to disable data-at-rest encryption for protected tenant data in any role.
. Attempt to disable data-at-rest encryption for protected tenant data through tenant-administrator interfaces (V1 and V2) and, through a provider-supported demonstration at the provider-internal vantage (V4) under the Evaluator Access Strategy (<<_evaluator_access_strategy>>) recorded in the ETR, through service-provider-administrator interfaces.
. Verify that no interface, in any role, performs the disable.
