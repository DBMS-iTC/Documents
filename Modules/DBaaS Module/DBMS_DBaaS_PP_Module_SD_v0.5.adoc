...
|0.5
|2026-08-11
|Refined broad Evaluation Activity scoping: representative testing now covers each distinct TSF path, with complete inventory review and ETR rationale, rather than every operational instance or permutation.
...
=== Negative-Property Evidence and the TOE/Trusted-Platform Boundary

The Evaluation Activities of this SD are predominantly negative, and how each negative property is evidenced is governed by two discriminators: what mediates the property, and proportionality to the assurance target. A property the TOE enforces through its own interfaces is evaluated as TOE behavior; a property of the infrastructure on which the service runs is a Trusted Platform property, addressed through assumptions, Operational Environment objectives, and scheme-accepted evidence, and is not established by testing the TOE. Several of this module's exclusion requirements are written to make this boundary self-evident: `FCS_CKM_DBAAS_EXT.1.2` and `FDP_BKP_EXT.1.1` prohibit an *interface* by which a service provider administrator obtains plaintext key material or a plaintext restore, so the claim is bound to TOE-mediated surface by construction. For such a property, the proportionate evidence is complete enumeration of the interfaces available in the evaluated configuration and an attempt of the excluded action through a representative of each distinct TSF path under <<_representative_test_scoping>> (native ADV_FSP.2 and ADV_ARC.1 territory), extended to a provider-supported demonstration at the provider-internal vantage (V4, see <<_evidence_vantages>>) where a relevant path is exposed only to provider roles, with each such reliance recorded in the ETR under the Evaluator Access Strategy (<<_evaluator_access_strategy>>).

The boundary is a limit on what the Evaluation Activities may claim, not only on what the evaluator can reach. These exclusions bind the TOE's interfaces; they neither assert nor are evaluated to establish that a Trusted Platform administrator cannot suppress or bypass a security function by acting through the infrastructure substrate (the memory, storage, network, or control plane the TOE does not own). That residual is held by `A.TRUSTED_PLATFORM_ADMIN`, which `OE.PROVIDER_ROLE_SEPARATION` keeps from extending to the Service Provider Administrator who is the threat agent of `T.KEY_DISCLOSURE_TO_PROVIDER_ADMIN`. An Evaluation Activity shall therefore confirm the absence of a TOE or service interface that performs the excluded action, and shall not attempt to demonstrate that the TOE prevents infrastructure-level suppression or bypass; an activity of the latter kind would evaluate an assumption and implicitly enlarge the TOE boundary. Architecture and interface review, rather than evaluator-conducted control-plane testing, is what holds this line, because it distinguishes a service interface from an infrastructure action where a live negative test against a replicated control plane can conflate the two. A provider environment that replicates the control plane is a permissible venue for a provider-supported demonstration, but its availability does not make evaluator-conducted control-plane penetration an expected method: that depth corresponds to the higher-assurance vulnerability families, and requiring it against an EAL2 package would be disproportionate to the assurance target and would invite findings against surface this PP-Module does not claim.

The discriminator is TOE mediation, not provider vantage, and a property the TSF genuinely performs is not excused from executed confirmation merely because it is reachable only through the provider-internal vantage (V4 in <<_evidence_vantages>>). Two consequences follow. First, verification of an update signature under `FPT_TUD_DBAAS_EXT.1.2` is TSF behavior: its negative leg (an artifact presented with an invalid or absent signature is refused before the software is placed into service) is confirmed by evaluator induction where the evaluated configuration exposes an update-submission interface, and otherwise by a provider-supported demonstration in a non-production instance under evaluator observation, recorded in the ETR. The demonstration is required, not conditional on convenience; only its modality follows the vantage. Second, an auditable event the TSF generates is confirmed at the channel the ST identifies (see <<_audit_channel_identification>>); where the triggering action is provider-initiated and is already staged as a provider-supported demonstration (an attempted plaintext restore or an attempted export of customer-controlled key material), the same demonstration confirms that the resulting record is produced and is visible to the affected tenant, and the evaluator does not separately reproduce the provider action. Correspondingly, the integrity of a service event stream implemented by the TSF is a TOE-mediated property evaluated as such, while the integrity of a stream delivered by the Trusted Platform is an Operational Environment property under `OE.TRUSTED_PLATFORM` and `OE.EXTERNAL_SERVICES`. In every case the rule is the same: test what the TOE mediates, assume what the Trusted Platform controls, and evidence the provider-only TOE-mediated negatives by demonstration proportionate to the EAL2 target.
...
. Use the representative-test scoping application note (<<_representative_test_scoping>>) to inventory the DBMS and management interfaces available to tenant or provider roles that expose, or could expose, plaintext master key material and select a representative from each group.
. Attempt to export, view, recover, or display the plaintext master key material through the selected representative interfaces.
. Where a relevant interface is restricted to provider roles, perform the corresponding representative attempt through a provider-supported demonstration under the Evaluator Access Strategy (<<_evaluator_access_strategy>>) and record the method and results in the ETR.
...
. Using a non-production customer-controlled test key whose material is known to the evaluator where the selected key model permits, induce a documented key authorization, lookup, or key-provider failure likely to generate diagnostic output.
. Apply the representative-test scoping application note (<<_representative_test_scoping>>) to review the error response and corresponding audit, status, diagnostic, backup-metadata, and management outputs from the exercised key operation.
. For every non-equivalent diagnostic TSF path identified in the TSS or operational guidance, induce one representative key-operation failure that reaches the path and review its output.
. For non-tenant-accessible diagnostics, use provider-supported observation under the Evaluator Access Strategy (<<_evaluator_access_strategy>>) and verify that the observation method and results are documented in the ETR.
...
. Use the representative-test scoping application note (<<_representative_test_scoping>>) to inventory the documented SQL, API, management, import/export, backup, restore, and tenant administration interfaces capable of accessing tenant data, and select representatives that cover every distinct Tenant Isolation SFP path.
. As Tenant B, attempt to access Tenant A's objects through the selected representative interfaces.
. Verify that all unauthorized attempts fail.

*Test 2: Metadata Isolation*

. As Tenant A, create objects with distinct names.
. Use the representative-test scoping application note (<<_representative_test_scoping>>) to inventory the tenant-visible system catalogs, metadata views, information schema views, management APIs, search interfaces, and monitoring views, and select representatives that cover every distinct metadata-access-control path.
. As Tenant B, query the selected representative interfaces.
...
. Confirm that the same provider-supported demonstration produces the service event stream record required by the module's auditable-events table for the attempted plaintext restore or export, and that the record is visible to the affected tenant, reusing the demonstration already staged in the preceding steps rather than separately reproducing the provider action (see <<_audit_channel_identification>>).
...
. Use the representative-test scoping application note (<<_representative_test_scoping>>) to inventory the claimed management functions, their authorized roles, and their interfaces, and select representatives that cover every distinct management-function authorization path.
. Perform each selected management-function representative as an authorized role using the interfaces available in the evaluated configuration.
. For selected management-function representatives restricted to provider roles that are not available to the evaluator, perform the authorized-role leg through a provider-supported demonstration under the Evaluator Access Strategy (<<_evaluator_access_strategy>>), recorded in the ETR; the unauthorized-role denial leg shall still be tested directly through tenant interfaces.
...
For `FMT_MOF.1/DBaaS-Disable`, the evaluator shall confirm, from the TSS and the functional specification, a complete inventory of the interfaces that manage audit generation and data-at-rest encryption, their authorized roles, and their TSF implementation paths. Applying the representative-test scoping application note (<<_representative_test_scoping>>) when selecting executed tests, the evaluator shall verify that the inventory identifies no documented interface that permits disabling either function in the evaluated configuration, consistent with the component's refinement and aligned to the no-interface idiom of `FCS_CKM_DBAAS_EXT.1.2` and `FDP_BKP_EXT.1.1`.
...
*Test 1: No Interface Disables Audit Generation*

. Confirm, from the TSS and the functional specification, the complete inventory of interfaces that manage audit generation and their role exposure.
. Apply the representative-test scoping application note (<<_representative_test_scoping>>) to select representatives that cover every distinct TSF path through which a request to disable audit generation is handled.
. Attempt to disable audit generation for TOE-required audit events through the selected tenant-administrator representatives (the tenant data plane V1 and tenant service control plane V2). Where the inventory identifies a relevant provider-only group, perform its representative attempt through a provider-supported demonstration at the provider-internal vantage (V4) under the Evaluator Access Strategy (<<_evaluator_access_strategy>>) and record the method and results in the ETR.
. Verify that the selected attempts are denied and that the complete inventory identifies no documented interface that performs the disable.
. The evaluator does not attempt to establish that infrastructure-level suppression of audit generation by a Trusted Platform administrator is prevented; that residual is held by `A.TRUSTED_PLATFORM_ADMIN` and is not evaluated as TOE behaviour (see <<_negative_property_evidence_and_the_toe_trusted_platform_boundary>>).

*Test 2: No Interface Disables Data-at-Rest Encryption*

. Confirm, from the TSS and the functional specification, the complete inventory of interfaces that manage data-at-rest encryption and their role exposure.
. Apply the representative-test scoping application note (<<_representative_test_scoping>>) to select representatives that cover every distinct TSF path through which a request to disable data-at-rest encryption is handled.
. Attempt to disable data-at-rest encryption for protected tenant data through the selected tenant-administrator representatives (V1 and V2). Where the inventory identifies a relevant provider-only group, perform its representative attempt through a provider-supported demonstration at the provider-internal vantage (V4) under the Evaluator Access Strategy (<<_evaluator_access_strategy>>) and record the method and results in the ETR.
. Verify that the selected attempts are denied and that the complete inventory identifies no documented interface that performs the disable.
. The evaluator does not attempt to establish that infrastructure-level suppression of data-at-rest encryption by a Trusted Platform administrator is prevented; that residual is held by `A.TRUSTED_PLATFORM_ADMIN` and is not evaluated as TOE behaviour (see <<_negative_property_evidence_and_the_toe_trusted_platform_boundary>>).
...
Where the ST identifies a service event stream as implemented by the TSF, the evaluator shall verify the TSS describes how the mechanism that protects the DBMS audit trail from unauthorised modification and deletion also protects the records of that stream to the same standard.

`FAU_STG.2/DBaaS` protects the DBMS audit trail and any service event stream implemented by the TSF. Where the ST identifies a service event stream as implemented by the TSF, its protection from unauthorised modification and deletion is evaluated here to the same standard as the DBMS audit trail; where the ST identifies the stream as delivered by the Trusted Platform, its integrity is an Operational Environment property under `OE.TRUSTED_PLATFORM` and `OE.EXTERNAL_SERVICES`, recorded with the Trusted Platform verification rather than tested as TOE behaviour. Tenant visibility and retrieval of service event stream records remain as defined by the module's auditable-events refinement and verified under <<_audit_channel_identification>>.
...
*Test 1: DBMS Audit Trail Deletion and Modification Prevention*

. As a tenant administrator, identify an audit record generated by the TOE.
. As a tenant administrator, through the tenant data plane (V1) and the tenant service control plane (V2), attempt to delete, modify, truncate, drop, or purge that record or its underlying audit storage, using documented tenant interfaces such as SQL DELETE, UPDATE, TRUNCATE, DROP, or PURGE commands, management APIs, or tenant audit tools.
. Verify that the operation is prevented.
...
*Test 3: Service Event Stream Deletion and Modification Prevention (TSF-implemented)*

. This test applies where the ST identifies a service event stream implemented by the TSF. Where the stream is delivered by the Trusted Platform, its integrity is recorded as an Operational Environment property under `OE.TRUSTED_PLATFORM` and `OE.EXTERNAL_SERVICES` per <<_audit_channel_identification>> and is not tested here.
. Identify a service event stream record generated by the TSF (for example, a key-unavailable enforcement transition, a recovery-mechanism use, an attempted plaintext restore, or an update deployment).
. Through a provider-supported demonstration at the provider-internal vantage (V4) under the Evaluator Access Strategy (<<_evaluator_access_strategy>>), recorded in the ETR, attempt to delete, modify, or suppress that record as a service provider administrator or service account through TOE interfaces.
. Verify that the operation is prevented. The evaluator does not attempt tenant-administrator deletion of the service event stream, which exposes no such interface (a tenant's relationship to the stream is read-only retrieval), nor does the evaluator attempt to establish that infrastructure-level suppression by a Trusted Platform administrator is prevented (held by `A.TRUSTED_PLATFORM_ADMIN`; see <<_negative_property_evidence_and_the_toe_trusted_platform_boundary>>).
...
. Present an artifact with an invalid or missing signature and verify that it is not placed into service and that the failure is audited. This negative leg is required, not conditional on convenience: where the evaluated configuration does not expose an update-submission interface through which the evaluator can present such an artifact, the service provider shall demonstrate signature rejection on a non-production instance under evaluator observation, recorded in the ETR (see <<_negative_property_evidence_and_the_toe_trusted_platform_boundary>>). Only the modality of the demonstration follows the vantage; the rejection itself shall be evidenced.
...
| V1, V2; V4 for any provider-restricted disclosure path
| Representative key-export and key-display attempts for every distinct TSF path; denial or metadata-only responses, with provider-supported demonstration where applicable.
...
| Representative diagnostic-producing key operations cover every distinct diagnostic TSF path; the error, audit, status, diagnostic, backup-metadata, and management outputs are reviewed for key-material disclosure, with provider-supported observation for non-tenant diagnostics.
...
| Representative cross-tenant data and metadata access attempts cover every distinct Tenant Isolation SFP path at V1; tenant provisioning and any evaluated sharing configuration are at V2.
...
| Representative management functions cover every distinct authorization path and are exercised as an authorized role; denial leg is tested directly from tenant vantages, with provider-supported authorized legs where required.

| FMT_MOF.1/DBaaS Tests 1–3 (Immutability and Role Restriction)
| V1, V2; V4 for any provider-only management path
| Representative tenant-administrator and provider-only attempts to disable audit or encryption cover every distinct TSF path; denial and the complete interface inventory are verified. Privilege-escalation denial is observed.
...
| V4
| Provider-supported demonstration of the update path on a revision-parity non-production instance, including signature verification and a required invalid-signature rejection (by evaluator induction where an update-submission interface is exposed, otherwise by provider demonstration recorded in the ETR).
