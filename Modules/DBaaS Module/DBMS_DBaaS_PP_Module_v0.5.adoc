...
+
The TOE shall be capable of exporting tenant-scoped audit records so that tenants can implement their retention policies using external audit storage solutions, through the audit export and query-role management functions of `FMT_SMF.1/DBaaS`; whether a given export or query capability is available to a tenant is determined by the roles the provider configures under those functions. The TOE must protect retained audit records from unauthorized modification and deletion while under TOE control. In a provider-operated service, retention obligations apply to both the provider (service-level audit) and the tenant (tenant-scoped audit), and to both audit channels defined by this module's auditable-events refinement: the ST identifies, for the DBMS audit trail and the service event stream separately, the retention treatment applicable to each and which records are exportable to tenants.
...
*FMT_MOF.1.1/DBaaS-Disable* The TSF shall restrict the ability to [_disable_] the functions [_audit generation and data-at-rest encryption for protected tenant data_] to [_no role in the evaluated configuration_]. *The TSF provides no interface, in any role, through which these functions can be disabled.*

*Application Note:* No role, including the Service Provider Administrator, may disable the identified functions in the evaluated configuration; the requirement is realized as the absence of any TOE interface that disables audit generation or data-at-rest encryption, consistent with the interface-scoped exclusions of `FCS_CKM_DBAAS_EXT.1.2` and `FDP_BKP_EXT.1.1`. This requirement binds the TOE's interfaces; it does not, and is not evaluated to, prevent a Trusted Platform administrator from suppressing these functions by acting through the infrastructure substrate outside the TOE boundary, which is addressed by `A.TRUSTED_PLATFORM_ADMIN` and `OE.PROVIDER_ROLE_SEPARATION`. CC Part 2 FMT_MOF.1 has a single element; the disable restriction and the provider-role restriction are therefore expressed as two iterations rather than as two elements of one component.
...
*FAU_STG.2.1/DBaaS* The TSF shall protect the stored audit records in the audit trail *and in any service event stream implemented by the TSF* from unauthorised deletion, *including deletion attempted through TOE interfaces by any role not authorised to delete audit records in the evaluated configuration*.

*FAU_STG.2.2/DBaaS* The TSF shall be able to [*prevent*] unauthorised modifications to the stored audit records in the audit trail *and in any service event stream implemented by the TSF*, *including modifications attempted through TOE interfaces by any role not so authorised*.

*Application Note:* This SFR is CC:2022 Part 2 FAU_STG.2 (Protected audit trail storage), iterated with the modification selection completed as "prevent". No role authorised in the evaluated configuration may delete or modify stored audit records through TOE interfaces. For the DBMS audit trail the salient adversary is a tenant administrator (for example, a privileged tenant database user attempting to delete, truncate, drop, or purge audit storage), and a service provider administrator where the provider holds a service interface to the trail; the TSF prevents both. A tenant's relationship to a service event stream is read-only retrieval through the tenant service control plane, so a tenant administrator has no TOE interface by which to delete or modify its records; for a service event stream implemented by the TSF the salient adversary is therefore a service provider administrator or service account, whose provider-initiated actions the stream is designed to record. Where the service event stream is delivered by the Trusted Platform, its integrity is an operational environment property under OE.TRUSTED_PLATFORM and OE.EXTERNAL_SERVICES; this requirement binds only records the TSF stores and reaches through TOE interfaces, and does not extend to infrastructure-level suppression by a Trusted Platform administrator, which is addressed by A.TRUSTED_PLATFORM_ADMIN. If audit records are exported to an external logging or storage service, the ST shall identify whether the export mechanism, external service, or both are included in the TOE; external services relied upon for storage immutability are addressed by OE.EXTERNAL_SERVICES unless claimed as TOE functionality.
...
All CC Part 2 components reused in this PP-Module are iterated with a `/DBaaS` label (`FDP_RIP.1/DBaaS`, `FDP_ACC.1/DBaaS`, `FDP_ACF.1/DBaaS`, `FRU_RSA.1/DBaaS`, `FIA_USB.1/DBaaS`, `FMT_SMR.1/DBaaS`, `FMT_SMF.1/DBaaS`, `FMT_MOF.1/DBaaS-Disable`, `FMT_MOF.1/DBaaS-Modify`, `FAU_SEL.1/DBaaS`, `FAU_STG.2/DBaaS`). Where the iterated component is also claimed by the Base PP, the iteration refines it for the multi-tenant managed-service context without altering the Base PP requirement itself, and the Base PP component remains claimed in its original form; where the component is a CC Part 2 component not used by the Base PP (for example, `FAU_STG.2` and `FMT_MOF.1`), the `/DBaaS` iteration introduces it for the DBaaS context, with no Base PP counterpart to alter. The extended components introduced by this PP-Module (`FCS_CKM_DBAAS_EXT.1`, `FDP_BKP_EXT.1`, `FRU_RSG_EXT.1`, `FPT_TUD_DBAAS_EXT.1`) are DBaaS-specific, are defined only where no CC Part 2 component provides the required behavior, and do not duplicate or contradict any Base PP or Crypto Module component. Cryptographic functionality is delegated entirely to the mandatory DBMS Cryptographic Functions Module.
...
|Mandatory dependency (provided by the Crypto Module)
