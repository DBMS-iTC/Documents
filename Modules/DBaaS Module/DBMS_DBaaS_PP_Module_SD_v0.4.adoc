...
. Verify that the transition to the key-unavailable enforcement state generates the audit record required by the module's auditable-events table.
...
. Verify that any continued access to protected data does not extend beyond the completed key-state enforcement interval, and that the TOE transitions to the key-unavailable enforcement state with the required audit record.
